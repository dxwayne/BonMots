\renewcommand{\footerfilename}{Commandline.tex}
...
IRAF uses its own notation for environment variables \index{IRAF!Environment Variables}.
The format consists of a keyword followed by a dollar-sign {\$}. The complete
list can be shown with the \dhl{show} command or using the cl commnad \dhl{osfn()} or
the iraf equivalent \dhl{iraf.osfn('var\$')} command. See \dhl{help osfn} for details.
The show command does not expaned any environment variables it contains, and you
can eventually get there using repeated show commands to reveal the constituent parts.

IRAF uses a notation of a underscore+close-parenthesis followed by a dot
parameter name \dhl{"\_).observatory"} \index{IRAF!\_)} to denote the parameter managed by
\dhl{epar package} or setting using \dhl{iraf.onedspec.observatory='kpno'}
within PyRAF scripts. Care needs to be exercised to avoid confusing 
parameters of other tasks with the same name. For example the task
\dhl{noao.observatory} does not cross fill the onedstds.observatory parameter.
