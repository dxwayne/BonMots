...
\newglossaryentry{band}{name={band},description={A dimension of multiple axis images, NAXIS >= 3. In IRAF, a subsets of information related to the data. In multispec spectra, band 1) is the final signal; 2) the raw signal before corrections; 3) the sky area; and 4) the SNR for the spectra.}}
...
\newglossaryentry{cl}{name={cl},description={IRAF ``Command Language'', the basis for IRAF scripting. Superceded by Extended Command Language (ecl), then by PyRAF -- where IRAF commands can be intermingled with python.}}

\newglossaryentry{combine}{name={combine},description={In IRAF the combination of data from two or more images.}}

\newglossaryentry{command}{name={command},description={In computing, an imperative subject to qualification by parameteres, to accept data from a source (stdin), and send results to an output (stdout). }}
...
\newglossaryentry{dispaxis}{name={dispaxis},description={The DISPAXIS keyword sets the expected direction of a spectrum's dispersion axis. 1='follow rows'; 2='follow columns'; 3=datacube. This is set with \dhl{epar apextract} command.}}

\newglossaryentry{ecl}{name={ecl},description={IRAF ``Extended Command Language'', Extended replacement for Command Language (ecl).}}
...
\newglossaryentry{foreign}{name={foreign},description={A foreign command is declared in login.cl at  logunuser.cl at startup. You can add your local commands to this list.}}
...
\newglossaryentry{package}{name={package},description={A collection of tasks, parameters, and help files within IRAF that must be loaded to access.}}
...
\newglossaryentry{pixel}{name={pixel},description={``Pixel Element'': the  single/smallest discrete element of a sensor. }}

\newglossaryentry{procedure}{name={procedure},description={A separately compiled/interpreted collection of cl/ecl statements appearing as a subroutine.}}
...
\newglossaryentry{program}{name={program},description={A collection of procedures, interpreted/compiled with the CL/ECL interface. May be processed with SPP to produce native executables or implemented with another language and used as a ``\dhl{foreign task}''}}
...
\newglossaryentry{stdin}{name={stdout},description={Unix: the file stream supplying input to agents reading the standard input stream. The sequence \dhl{grep < l.l} searches each line of l.l using the grep command.}}

\newglossaryentry{stderr}{name={stdout},description={Unix: the file stream where error messages are written, to the terminal or redirected to a file.}}

\newglossaryentry{stdout}{name={stdout},description={Unix: the file stream where nominal messages are written, to the terminal or redirected to a file.}}
...
\newglossaryentry{trace}{name={trace},description={The path of a star's image across a sensor in the same beam. This usually follows a curve due to astigmatisms.}}

\newglossaryentry{whitespace}{name={whitespace},description={A character, usually non-printing like space, tab etc, used to separate lexical parts of a line into individual tokens. The meaning of a token is taken from the surrounding context.}}
