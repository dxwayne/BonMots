...
%% APPENDIXes
% cleantex && pdflatex BonMots.tex && makeglossaries BonMots && pdflatex BonMots.tex && evince BonMots.pdf
%% APPENDIXEND
...
\\
``If you pay very close attention to the noise, the signal will take care if itself.''
...
Quick Overview of the file system layout we recommend. This example is used throughout.
...
I do not use IRAF/PyRAF for photometry. I use Sextractor instead, together
with Astrometry.net and PostgreSQL. That said, this document focuses on
spectroscopy.

IRAF/PyRAF is in transition, with several packages showing variations in their
...
Data is in two quintessential places: 1) The computer that collects the data
from the instrument, and 2) the computer you are using to reduce the data.
...
.2 Desktop.
.3 Today - where all ``tonight's'' observations go.
}
...
contains loosely related information. For example peruse \dhl{/etc},
...
sub-directories that hold common information for different sites or
...
See the complicated example of how to deal with special camera
images.
...
\input{ReductionSpectra.tex}



\section{IRAF ``at-files'' or ``@'' Tricks}

The \dhl{at-cost} \llbox{@} \index{"@} symbol has been co-opted by IRAF to indicate
use a file where each line is a filename for the command to operate upon.
It is a 'list-of-files' type file.

Some practitioners recommend using lists and a text editor to manage
cases where work is done to a set of input files, that are save with a new
with a new name. To do this they use \dhl{ls} to make a list. Then edit the
list and add/change the name for the change. This is very tedious.

The steps we usually follow are to make the master files. If you
prepend a single letter to the old filename you can perform trimmig,
cosmic ray correction, zero subtraction, dark subtraction,
flat-fielding quite quickly.

Lets set the IMAGETYP for the files to be something that matches the IRAF
vocabulary.

It is created along the lines of:\\
~~~\dhl{! ls -1 *Zero*fits > l.l}\\
taken then used along the lines of: \\
~~~hedit @l.l IMAGETYP zero add+ update+ del- ver- show-

to set the file type of the 'zero exposure files' to have a FITS keyword
IMAGETYP set to 'zero' to satisfy the \dhl{zerocombine} command requirements.
The IRAF vocabulary follows:

\begingroup \fontsize{10pt}{10pt}
\selectfont
%%\begin{Verbatim} [commandchars=\\\{\}]
\begin{verbatim} 
# NAME                IRAF VOCABULARY
OBJECT                  object
DARK                    dark
"PROJECTOR FLAT"        flat
"SKY FLAT"              other
...
The ``\dhl{trick}'' is to set all files to \dhl{object}, then set the subset
to their proper names. 



During the 1980 command lines were growing longer and longer. We
simply had more files to work with. So rather than having to type in a
long list of files the ``seldom-used'' at-cost symbol
``{\color{verbcolor}{\verb#@#}}'' was conscripted to mean a ``file''
that contained a ``list-of-files'' with ``one-filename-per-line''. The
command interpreters were hacked to pause, open that file, and load up
the developing parameter list (argv for you C programmers) with the
contents of that file. Hey, it works, and we love it.
\ltodo{rework}{The load up paremeter the developing list parts needs
  rethinking}
...
\gls{IRAF} uses a {\color{verbcolor}{\verb#@filename#}} by taking a
...
\Gls{analysis} \menu Image Servers \menu DSS .... choose plates, fill in the
...
\cite{2005ASPC..347...29T}, the Aladin\footnote{It is pronounced Al -
  A - Din not like the Disney movie. I asked a french astronomer. Add CDS to search-engine queries
to disambiguate.}
...
print "Trim overscan regions from special camera files.  [1:1368,1:1364]"
files *fits > l.l
...
%%\printglossary
%%\IfFileExists{BonMots.gls}{%
%%   \input{BonMots.gls}
%%}

\clearpage
%\printindex
\IfFileExists{BonMots.ind} {%
  \input{BonMots.ind}
}
