...
A collection of handy things to know right off the bat. IRAF is the
``Image Reduction and Analysis Facility'' it is a collection of
programs, not a single executable on its own. This approach saves
'main' memory.  The machine being used to compose this document has
190 MB or main memory, roughly 48 times the size of my early
University's computer. IRAFs executables are managed by \dhl{Command Language}
(\gls{cl}), the \dhl{Extended Command Language} (\gls{ecl}), and \dhl{PyRAF} (the
program is called \dhl{pyraf}). PyRAF is a customized (hacked) python
interpreter that makes use of iraf.py -- that in turn may call upon
the original \dhl{cl} or \dhl{ecl} that are \dhl{bash} scripts to start a
traditional \dhl{iraf} processing session.

\textbf{\emph{To exit}} \dhl{cl} or \dhl{ecl} type \dhl{logout} at the prompt.\index{exiting!cl/ecl}

\textbf{\emph{To exit}} \dhl{PyRAF} use \dhl{.ex} at the prompt.\index{exiting!pyraf}

\textbf{\emph{To exit}} \dhl{bash} use \dhl{exit} at the prompt.\index{exiting!bash}
...
%\setcounter{enumi}{N}
   \item   IRAF uses commands.
   \item   IRAF's GUI like interface called \dhl{epar} allows you to set the myrid of variables for a 'command'.
   \item   IRAF uses Unix Commands:
\vspace{-.15cm}
\begin{enumerate}\addtolength{\itemsep}{-0.5\baselineskip}
%\setcounter{enumi}{N}
   \item   None are built in, but are declared as \dhl{foreign}. You can add to \gls{foreign} commands.
   \item   Non-foreign commands can be run by \dhl{excaping} to the shell. Use the exclaimation point \llbox{!}
...
\end{enumerate}
   ...
   \item   Commands can be run as their python funcional equivalents:
\end{enumerate}
...
Most files are a ``\dhl{single extention file}''. There is only one section with
a data array. However! FITS permits ``\dhl{multiple extension files}'' or MEFs.
The \dhl{combine} tasks will add an extention each time they are run, and following
...
%\setcounter{enumi}{N}
  ...
  things.
   ...
   \item   If you are really stuck use \mousebox{Ctrl-Z} to pause the task and kill it with a: \\
...
(specifically the conditions/coefficents of polynominals) in a file
...
Unix and IRAF/PyRAF parse command lines by breaking the line into parts
using spaces.\\
 \textbf{\emph{NOTE}}: \dhl{BREAK THE HABIT PUTTING OF SPACES IN FILENAMES}.

Avoid using spaces, pluses/minuses, punctuation (ampersand, asterisks,
colons and semicolons in particular), and other non
[A-Za-z0-9]. Always start a filename with a letter. Keep the names as
short as possible.

Trick: prepend a single letter and underscore (z\_ for zero subtracted files for
...
\begin{quote}
Indicating to use images \dhl{Zero*} as imput; use
